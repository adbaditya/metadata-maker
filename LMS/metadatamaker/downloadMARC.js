...
function fillWebURL(record,head,fieldFunc,subfieldFunc) {
	...
	var web_url = fieldFunc(tag,'4','0',[subfieldFunc('u',record.web_url),subfieldFunc('3','Full text')]);
...
function fillSubjectCategory(record,head,fieldFunc,subfieldFunc) {
	...
	if (record.subjects.length > 0) {
		...
		for (var index = 0; index < record.subjects.length; index++) {
			...
			var new_content = fieldFunc(tag,' ',' ',subject_category_subfields);
		...
		return returnMultipleEntries(subject_categories_directory,subject_catagories,head);
	}
	else {
		return head !== null ? ['','',head] : '';
	}
...
//			subject_subfields.push(subfieldFunc('a',record.subjects[index]['id_number'].substring(0,3)));
			...
			var new_content = fieldFunc(tag,'0','7',subject_subfields);
		...
		return returnMultipleEntries(subject_directory,subjects,head);
	}
	else {
		return head !== null ? ['','',head] : '';
	}
...
function downloadXML(record,institution_info) {
	...
	text += '  <controlfield tag="007">' + controlfield007 + '</controlfield>\n'
	...
	text += fillSubjectCategory(record,null,createMARCXMLField,createMARCXMLSubfield);
	...
	text += fillSubjects(record,null,createMARCXMLField,createMARCXMLSubfield);
