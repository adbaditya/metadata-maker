...
async function fetchFromPerplexityWithImages(textQuery, base64Images = []) {
    try {
        ...
        const response = await fetch('https://metadata-maker.adb-aditya.workers.dev/gemini', {
            method: 'POST',
            headers: {
    ...
    try {
        ...
        if (data && data.success && data.result) {
            ...
            if (data.result.candidates &&
                ...
                data.result.candidates[0].content.parts &&
                data.result.candidates[0].content.parts[0]) {

                let content = data.result.candidates[0].content.parts[0].text;
                ...
                try {
                    ...
                    console.log('✅ Successfully parsed Gemini data:', parsedData);
                    return {
                        choices: [{ message: { content: JSON.stringify(parsedData) } }]
                    };
                } catch (parseError) {
                    console.error('Error parsing Gemini content:', parseError);
                    console.log('Content that failed to parse:', content);
                    return null;
                }
            }
            // ✅ Handle Perplexity response structure (fallback)
                ...
                try {
                    ...
                    console.error('Error parsing Perplexity content:', parseError);
                    console.log('Content that failed to parse:', content);
                    return null;
                }
            }
        ...
        console.log('No valid content structure found in response');
        return null;
    } catch (error) {
...
Return ONLY this JSON format (no explanations, no citations):
{
    ...
    try {
        ...
        console.log('🔍 Stage 1 raw response:', response);

        if (response && response.choices?.[0]?.message?.content) {
            const content = response.choices[0].message.content;
            console.log('📄 Stage 1 content:', content);

            const basicInfo = tryParseWithFallbacks(content);
            console.log('✅ Stage 1 parsed:', basicInfo);

            // ✅ FIX: Check if we got EITHER title OR isbn (not both required)
            if (basicInfo && (basicInfo.title || basicInfo.isbn)) {
                console.log('✅ Stage 1 extracted valid info:', basicInfo);
                return basicInfo;
            } else {
                console.log('⚠️ Stage 1 extracted data but missing both title and ISBN:', basicInfo);
                // Still return the data - let the caller decide what to do
                return basicInfo;
            }
        }

        console.log('❌ No valid response from Stage 1');
        return null;
    } catch (error) {
...
async function generateMetadataWithAiImages() {
    ...
    try {
        ...
        let basicInfo = await extractBasicInfoFromImages(base64Images);

        if (!basicInfo || (!basicInfo.title && !basicInfo.author)) {
            console.log('⚠️ Could not extract title or author, trying direct metadata search...');
            // Instead of throwing error, continue with empty basicInfo
            // This allows the Stage 2 search to still work with user inputs
            basicInfo = {
                isbn: null,
        ...
        console.log('✅ Stage 1 complete, proceeding with:', basicInfo);
